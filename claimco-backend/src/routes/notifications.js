...
router.get("/", requireAuth, (req, res) => {
    ...
    WHERE n.recipient_id = ? ORDER BY n.created_at DESC, n.rowid DESC
  `).all(req.userId);
...
router.post("/:id/read", requireAuth, (req, res) => {
    db.prepare("UPDATE notifications SET read_at = datetime('now') WHERE id = ? AND recipient_id = ?").run(req.params.id, req.userId);
    res.json({ ok: true });
});
