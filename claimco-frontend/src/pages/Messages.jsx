    ...
    return (
        <div className="content">
            ...
            {!error && conversations.length === 0 ? (
                <div className="empty-note"><MessageCircle size={18} /> No chats yet. Message a seller about an item to start one.</div>
            ) : (
                <div className="conversation-list">
                    {conversations.map((conversation) => (
                        <Link className="conversation-item" key={conversation.id} to={`/chat/${conversation.id}`}>
                            <div className="conversation-copy">
                                <div className="conversation-person">{conversation.otherUser.name} · {conversation.item.title}</div>
